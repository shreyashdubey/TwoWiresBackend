...
// Delete Education
router.delete(
  "/delete-education/:educationId/:userId",
  authMiddleware.isAuthenticated,
  async (req, res) => {
    ...
    try {
      ...
      const user = await User.findByIdAndUpdate(education.user, {
...
router.get("/get-all-education", async (req, res) => {
  try {
    ...
    const totalEducationEntries = await Education.countDocuments({
      user: user,
      isDeleted: false,
    });
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      await User.findByIdAndUpdate(existingExperience.user, {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const totalExperienceEntries = await Experience.countDocuments({
        user: userId,
        isDeleted: false,
      });

      const totalPages = Math.ceil(
        totalExperienceEntries / pageOptions.pageSize
      );
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(201).json({
        message: "Skill entry added successfully",
        success: true,
        skill: newSkill,
      });
    } catch (error) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      await User.findByIdAndUpdate(existingSkill.user, {
        $pull: { skill: skillId },
      });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const skillEntries = await Skill.find({ user: userId, isDeleted: false })
        .skip(skip)
        .limit(pageOptions.pageSize);

      const totalSkillEntries = await Skill.countDocuments({
        user: userId,
        isDeleted: false,
      });
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (language) {
        ...
        return res
          .status(400)
          .json({ message: "Language not provided in the request body" });
      }
      await user.save();
      res.status(200).json({
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (languageIndex === -1) {
        return res
          .status(404)
          .json({ message: "Language not found in proficientLanguages" });
      }
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (currentIndustry) user.currentIndustry = currentIndustry;
      if (currentLocation) user.currentLocation = currentLocation;
      if (isMailPublic !== undefined) user.isMailPublic = isMailPublic;
      ...
      res.status(200).json({
        ...
        username: user.username /* ...other fields */,
      });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal Server Error" });
